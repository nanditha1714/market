...
export default function App() {
      ...
      if (id) {
        ...
        if (record) {
          try {
            ...
            parsedDash.pdf_url = record.pdf_url;
            parsedDash.dashboard_pdf_url = record.dashboard_pdf_url;
            parsedDash.report_pdf_url = record.report_pdf_url;
            parsedDash.payment_status = record.payment_status;
            parsedDash.email_status = record.email_status;
  ...
  const handleLogin = useCallback((userInfo) => {
    ...
    setUser(userInfo);
    // Asynchronously trigger welcome email
    if (userInfo.email) {
      sendWelcomeEmail(userInfo.email, userInfo.name || '').catch(err => {
        console.warn('Welcome email triggering failed:', err);
      });
    }
    window.location.hash = '#/survey';
  }, []);
  ...
  return (
    <>
      ...
      <Suspense fallback={<LoadingScreen />}>
        {screen === SCREENS.LOGIN && <LoginPage onLogin={handleLogin} initialGoogleUser={initialGoogleUser} onOpenAbout={() => setShowAboutSidebar(true)} />}
        {screen === SCREENS.SURVEY && <SurveyPage user={user} onComplete={handleSurveyComplete} onReset={handleReset} onOpenAbout={() => setShowAboutSidebar(true)} />}
        {screen === SCREENS.LOADING && <LoadingScreen />}
        {screen === SCREENS.DASHBOARD && <Dashboard data={dashData} user={user} answers={answers} onReset={handleReset} onOpenAbout={() => setShowAboutSidebar(true)} />}
      </Suspense>

      {showAboutSidebar && <AboutSidebar onClose={() => setShowAboutSidebar(false)} />}
    </>
  );
}
